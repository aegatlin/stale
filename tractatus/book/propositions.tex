...
T & F &\\
F & F & T
\end{array}'' is a propositional sign.
...
\begin{array}{ccc}
\text{(TTTT)($p, q$)} & \text{Tautology} & \text{(if $p$ then $p$, and if $q$ then $q$) [$p \Implies p \DotOp q \Implies q$]}\\
\text{(FTTT)($p, q$)} & \text{in words:} & \text{Not both $p$ and $q$. [$\Not{(p \DotOp q)}$]}\\
\text{(TFTT)($p, q$)} & \text{"} & \text{If $q$ then $p$. [$q \Implies p$]}\\
\text{(TTFT)($p, q$)} & \text{"} & \text{If $p$ then $q$. [$p \Implies q$]}\\
\text{(TTTF)($p, q$)} & \text{"} & \text{$p$ or $q$. [$p \lor q$]}\\
\text{(FFTT)($p, q$)} & \text{"} & \text{Not $q$. [$\Not{q}$]}\\
\text{(FTFT)($p, q$)} & \text{"} & \text{Not $p$. [$\Not{p}$]}\\
\text{(FTTF)($p, q$)} & \text{"} & \text{$p$ or $q$, but not both. [$p \DotOp \Not{q} : \lor : q \DotOp \Not{p}$]}\\
\text{(TFFT)($p, q$)} & \text{"} & \text{If $p$, then $q$; and if $q$, then $p$. [$p \equiv q$]}\\
\text{(TFTF)($p, q$)} & \text{"} & \text{$p$}\\
\text{(TTFF)($p, q$)} & \text{"} & \text{$q$}\\
\text{(FFFT)($p, q$)} & \text{"} & \text{Neither $p$ nor $q$. [$\Not{p} \DotOp \Not{q}$ or $p \BarOp q$]}\\
\text{(FFTF)($p, q$)} & \text{"} & \text{$p$ and not $q$. [$p \DotOp \Not{q}$]}\\
\text{(FTFF)($p, q$)} & \text{"} & \text{$q$ and not $p$. [$q \DotOp \Not{p}$]}\\
\text{(TFFF)($p, q$)} & \text{"} & \text{$p$ and $q$. [$p \DotOp q$]}\\
\text{(FFFF)($p, q$)} & \text{Contradiction} & \text{($p$ and not $p$; and $q$ and not $q$.) [$p \DotOp \Not{p} \DotOp q \DotOp \Not{q}$]}\\
\end{array}
...
application of the operation $\mbox{(-- -- -- -- --T)}(\xi, \fourdots)$ to
...
\PropositionE{5.502}
{Therefore I write instead of ``$\mbox{(-- -- -- -- --T)}(\xi, \fourdots)$'',
``$N(\overline{\xi})$''.
